...
const extractTone = (guidelines: string): 'professional' | 'casual' | 'enthusiastic' | 'formal' | 'friendly' => {
  const lowerGuidelines = guidelines.toLowerCase();
  
  if (lowerGuidelines.includes('professional') || lowerGuidelines.includes('business') || lowerGuidelines.includes('formal')) {
    return 'professional';
...
const generateEmailContent = (topic: string, tone: string, style: string): string => {
  const greeting = tone === 'formal' ? 'Dear Valued Client,' : 'Hi there,';
